...
if [ -n "$RUNVARCALLSBATCH" ]; then
	...
	NAME=$(echo ${DIR[@]}|sed 's/ /_/g')
  	...
  	for i in $BATCHES; do
		echo "[NOTE] Batch $i"
    	JOBID=$( $QSUB $ARMED -r -d -k $CONFIG -t ${TASK_VAR} -i $INPUT_VAR  -e .$ASR.bam -n $NODES_VAR \
        	-c $CPU_VAR -m $MEMORY_VAR"G" -w $WALLTIME_VAR \
        	--postcommand "${NGSANE_BASE}/mods/gatkSNPs2.sh -k $CONFIG \
                        -i <FILE> -t $CPU_VAR \
                        -r $FASTA -d $DBROD -o $OUT/$TASK_VAR/$NAME/ -n $NAME$i \
                        -H $HAPMAPVCF -L $i " 
		) && echo -e "$JOBID"
	...
	echo "[NOTE] wait for jobs $JOBIDS"
   	$QSUB $ARMED -r -k $CONFIG -t $TASK_VAR -i $OUT/$TASK_VAR/$NAME-i -e .vcf $JOBIDS \
