...
if [[ $(NGSANE_CHECKPOINT_TASK) == "start" ]]; then
    ...
    if [ -n "$FITHIC_START_FROM_FRAGMENTPAIRS" ]; then 
        ...
        RUN_COMMAND="python ${NGSANE_BASE}/tools/fithic-fixedBins/fithicCountInteractions.py $FITHIC_START_FROM_FRAGMENTPAIRS --verbose $FITHIC_2DMATRIX $TADBIT --mappability=$MAPPABILITY --resolution=$HIC_RESOLUTION --chromsizes=$GENOME_CHROMSIZES $FITHIC_CHROMOSOMES --outputDir=$OUTDIR/$SAMPLE --outputFilename $SAMPLE $DATASETS > $OUTDIR/$SAMPLE.log"
