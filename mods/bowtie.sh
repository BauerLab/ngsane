...
if [[ $? -eq 0 ]] && [ $PAIRED == "0" ]; then
    ...
    RUN_COMMAND="$GZIP -dc $f | bowtie $RG $BOWTIEADDPARAM --tryhard --best --strata --time --threads $THREADS -v 3 -m 1 --un $MYOUT/${n/%$READONE.$FASTQ/.$UNM.fq} --max $MYOUT/${n/%$READONE.$FASTQ/.$MUL.fq} --sam $BOWTIE_OPTIONS ${FASTA/.${FASTASUFFIX}/} - $MYOUT/${n/%$READONE.$FASTQ/.$ALN.sam}"
else
    echo "[NOTE] unzip fastq files"
    $GZIP -cd $f > $f.unzipped
    $GZIP -cd ${f/$READONE/$READTWO} > ${f/$READONE/$READTWO}.unzipped
    RUN_COMMAND="bowtie $RG $BOWTIEADDPARAM --tryhard --best --strata --time --threads $THREADS -v 3 -m 1 --un $MYOUT/${n/%$READONE.$FASTQ/.$UNM.fq} --max $MYOUT/${n/%$READONE.$FASTQ/.$MUL.fq} --sam $BOWTIE_OPTIONS ${FASTA/.${FASTASUFFIX}/} -1 $f.unzipped -2 ${f/$READONE/$READTWO}.unzipped $MYOUT/${n/%$READONE.$FASTQ/.$ALN.sam}"
fi
echo $RUN_COMMAND
eval $RUN_COMMAND
...
TAGDIRECTORY=$MYOUT/${n/%$READONE.$FASTQ/_homer}
mkdir -p $TAGDIRECTORY
makeTagDirectory $TAGDIRECTORY $MYOUT/${n/%$READONE.$FASTQ/.$ASD.bam}

echo "********* create Bigwig"
makeUCSCfile $TAGDIRECTORY -o $MYOUT/${n/%$READONE.$FASTQ/.bw} -bigWig $GENOME_CHROMSIZES -norm 1e6 -fsize 1e20 -fragLength 300
