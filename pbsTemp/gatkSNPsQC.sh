...
# Errors
for i in "error" "no such file" "file not found" "gatkSNPsQC.sh: line"
do
  var=`grep -i "$i" $QOUT/* | cut -d ":" -f 1 | sort -u | wc -l`
  if [ "$var" = "0" ]; then
...
# Steps
echo ">>>>>>>>>> Progress"
for i in "call SNPs" "mask intervals for the SNP calling later on" "hard filter SNPs" "make index for IGV"
do
