...
#        --postcommand "$HISEQINF/pbsTemp/hiclibCorrelate.sh $HICLIBADDPARM -k $CONFIG --threads $CPU_HICLIB --memory $(expr $MEMORY_HICLIB - 1 ) --enzymes "$HICLIB_RENZYMES" --outdir $OUT/<DIR>/$TASKHICLIB --fastq <FILE> --fastqName <NAME>"
